    ...
    public class Tests
    {
        ...
        public void Example()
        {
            Assert.IsTrue(new[] {"КОТ", "ТОТ", "ТОН"}.SequenceEqual(
                FindPath("КОТ", "ТОН", dictionary).Value));
        }
        ...
        public void FlyElephantExample()
        {
            Assert.AreEqual("муха-мура-кура-кара-каре-кафе-кафр-каир-каин-клин-клон-слон",
                string.Join("-", FindPath("муха", "слон",
        ...
        {
            Assert.IsTrue(new[] {"КОТ"}.SequenceEqual(FindPath("КОТ", "КОТ", dictionary).Value));
        }

        [TestMethod]
        public void ArgsDifferentLengths()
        {
            Assert.IsFalse(FindPath("КОТ", "НОТА", dictionary).HasValue);
        }

        [TestMethod]
        public void StartWordNotInDictionary()
        {
            Assert.IsFalse(FindPath("КОТ", "ТОН", new[] {"ТОН", "НОТА", "КОТЫ", "РОТ", "РОТА", "ТОТ"}).HasValue);
        }

        [TestMethod]
        public void EndWordNotInDictionary()
        {
            Assert.IsFalse(FindPath("КОТ", "ТОН", new[] {"КОТ", "НОТА", "КОТЫ", "РОТ", "РОТА", "ТОТ"}).HasValue);
        }

        [TestMethod]
        public void StartWordIsEmpty()
        {
            Assert.IsFalse(FindPath("", "ТОН", dictionary).HasValue);
        }

        [TestMethod]
        public void EndWordIsEmpty()
        {
            Assert.IsFalse(FindPath("КОТ", "", dictionary).HasValue);
        }

        [TestMethod]
        public void WordsAreEmpty()
        {
            Assert.IsTrue(new[] {""}.SequenceEqual(FindPath("", "", new[] {""}).Value));
        }
        ...
        {
            Assert.IsTrue(new[] {"КОТ", "ТОТ", "ТОН"}.SequenceEqual(
                FindPath("КОТ", "ТОН", new[] {"КОТ", "ТОН", "НОТА", "КОТЫ", "РОТ", "РОТА", "ТОТ", "ТОТ"}).Value));
        }

        [TestMethod]
        public void EmptyDictionary()
        {
            Assert.IsFalse(FindPath("КОТ", "КОТ", new string[] {}).HasValue);
        }

        [TestMethod]
        public void EmptyDictionary2()
        {
            Assert.IsFalse(FindPath("КОТ", "ТОН", new string[] {}).HasValue);
        }

        [TestMethod]
        public void SingleLetterWords()
        {
            Assert.IsTrue(new[] {"a", "b"}.SequenceEqual(FindPath("a", "b", new[] {"a", "b", "c"}).Value));
        }

        [TestMethod]
        public void SingleLetterWords2()
        {
            Assert.IsFalse(FindPath("a", "b", new[] {"c"}).HasValue);
        }
    }
}
